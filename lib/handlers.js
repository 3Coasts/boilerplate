...
Handlers.prototype.putEntityId = function putEntityId(req, res, next) {
  ...
  Model.update({ slug: req.params.id }, omit(req.body, handlers.readOnly))
    .exec(function(err, model) {
      if (err) return next(err);

      var query = Model.findOne({ slug: req.params.id });
...
Handlers.prototype.deleteEntityId = function deleteEntityId(req, res, next) {
  this.Model.destroy({ slug: req.params.id }, function(err) {
    if (err) return next(err);
