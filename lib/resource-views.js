...
function eachResource(resource) {
  ...
  var attributes = toArray(mapValues(resource.model.attributes, function(value, key) {
    ...
    return {
      ...
      model: value.model ? { title: find(resources, { name: value.model }).view.titleField, name: value.model } : false,
    ...
    function(files, cb) {
      async.each(files, function(file, cb) {
        fs.readFile(templatePath + file, { encoding: 'utf8' }, function(err, template) {
          ...
          fs.writeFile(clientViewPath + file, hbTemplate(viewConfig), cb);
        })
      }, cb);
    },
